  ...
  return (
    <ApexScreen>
      <main className="page-frame track-page">
        ...
        <div className="track-layout"><div className="track-map"><OfficialCircuitImage imageUrl={track.layoutPath} name={track.name} verified={track.layoutStatus === 'verified'} priority /></div><div className="track-metrics"><Metric label="Length" value={track.lengthKm === null ? 'Unavailable' : `${track.lengthKm.toFixed(3)} km`}/><Metric label="Race distance" value={track.laps === null ? 'Unavailable' : `${track.laps} laps`}/><Metric label="Pit loss" value={track.pitLossSec ? `~${track.pitLossSec.toFixed(1)}s` : 'Unavailable'} detail="estimated"/><Metric label="DRS zones" value={track.drsZones.length || 'Unavailable'}/></div></div>
        <p className="track-map-source">{track.layoutStatus === 'verified' && track.layoutSourceUrl ? <a href={track.layoutSourceUrl} rel="noreferrer">Verified Formula 1 circuit asset ↗</a> : 'Official map unavailable. No substitute drawing shown.'}</p>
