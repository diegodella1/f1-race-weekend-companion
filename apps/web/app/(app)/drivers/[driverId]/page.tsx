  ...
  return (
    <ApexScreen>
      <main className="page-frame driver-detail">
        <header style={{ '--team-color': driver.teamColor ?? '#9BA3AF' } as CSSProperties}><p className="eyebrow">P{driver.position ?? '—'} · {driver.teamName}</p><h1>{driver.code}</h1><span>{driver.fullName}</span></header>
        ...
        {rival ? <div className="driver-actions"><Link className="primary-action" href={`/compare?session=${encodeURIComponent(session.id)}&a=${encodeURIComponent(driver.driverId)}&b=${encodeURIComponent(rival.driverId)}`}>Compare with {rival.code}</Link><Link href={`/strategy?a=${encodeURIComponent(driver.driverId)}&b=${encodeURIComponent(rival.driverId)}`}>Open strategy</Link></div> : null}
      </main>
    </ApexScreen>
  );
}
